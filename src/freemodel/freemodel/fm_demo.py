#!/usr/bin/env python3
"""
fm_demo.py  -  Waypoint Tracking + Leader/Follower Comparison
...
class FMDemo(Node):
    def __init__(self):
        ...
        self.out_dir = self.declare_parameter("out_dir", "/root/so100_ws/freemodel_out").value
        ...
        if len(wp_flat) < 2 * self.num_waypoints:
            raise RuntimeError(f"waypoints_flat length {len(wp_flat)} < 2*num_waypoints={2*self.num_waypoints}")
        ...
        self.get_logger().info(
            f"[fm_demo] rate_hz={self.rate_hz:.1f}  Ts={Ts:.3f}  control_scale={self.control_scale}  use_dq={self.use_dq}"
        )
        self.get_logger().info(f"[fm_demo] settle_time_s={self.settle_time_s}")
        self.get_logger().info(f"[fm_demo] waypoints={self.waypoints}")
        self.get_logger().info(f"[fm_demo] hold_steps={ [self.hold_steps]*len(self.waypoints) } (hold_secs each)")
        ...
        self.get_logger().info(f"[fm_demo] ||K* - K_learned||_F = {np.linalg.norm(self.K_star - self.K_learn):.6f}")

        # --- Topics
        self.sub_l = self.create_subscription(JointState, f"{self.ns_leader}/joint_states", self.cb_leader, 10)
        self.sub_f = self.create_subscription(JointState, f"{self.ns_follower}/joint_states", self.cb_follower, 10)

        self.pub_l = self.create_publisher(Float64MultiArray, f"{self.ns_leader}/arm_position_controller/commands", 10)
        self.pub_f = self.create_publisher(Float64MultiArray, f"{self.ns_follower}/arm_position_controller/commands", 10)
        ...
        self.get_logger().info(f"[fm_demo] Sending q_init={np.array(self.waypoints[0])} during settle...")
    ...
    def step(self):
        ...
        if self.wp_count >= self.hold_steps:
            ...
            if self.wp_idx >= len(self.waypoints):
                self.get_logger().info("[fm_demo] Done - saving logs and plots.")
                self.save_and_plot()
                self.timer.cancel()
                return
            self.q_des = np.array(self.waypoints[self.wp_idx], dtype=float)
            self.wp_count = 0
            self.get_logger().info(f"[fm_demo] -> waypoint {self.wp_idx+1}/{len(self.waypoints)}  q_des={self.q_des}")
    ...
    def save_and_plot(self):
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for k in range(len(t)):
                w.writerow([
                    ...
                    float(leader_q[k,0]), float(leader_q[k,1]), float(leader_dq[k,0]), float(leader_dq[k,1]),
                    float(foll_q[k,0]), float(foll_q[k,1]), float(foll_dq[k,0]), float(foll_dq[k,1]),
                ])
        ...
        self.get_logger().info(f"[fm_demo] CSV   -> {csv_path}")
        self.get_logger().info(f"[fm_demo] Plot  -> {out_png}")
